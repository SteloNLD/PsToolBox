Function Log-Start{
  ...
  .PARAMETER ScriptName
    Name of the running script which will be written in the log. Example: HelloWorld
  .PARAMETER ScriptMaintainer
    Maintainer name of the running script which will be written in the log. Example: John.Doe@Dummy.nl
  .INPUTS
    Parameters above
  .OUTPUTS
    Log file created
  .NOTES
    ...
    Purpose/Change: BUGFIX: Changed New-Item -Path $LogPath -Value $LogName -ItemType File  to New-Item -Path $LogPath -Name $LogName -ItemType File | Out-Null

    Version:        1.3
    Author:         Sten Lootens
    Creation Date:  29/11/2016
    Purpose/Change: Added the ScriptName and ScriptMaintainer params

    Version:        1.4
    Author:         Sten Lootens
    Creation Date:  9/12/2016
    Purpose/Change: BUGFIX changed $sFullPath to use Join-Path
  .EXAMPLE
  ...
  Param (
			...
			[Parameter(Mandatory=$false)][string]$ScriptMaintainer, 
  ...
  Process{
    ...
    Add-Content -Path $sFullPath -Value "***************************************************************************************************"
    Add-Content -Path $sFullPath -Value ""
    if ($ScriptName -ne $null) { Add-Content -Path $sFullPath -Value "$ScriptName"}
    if ($ScriptMaintainer -ne $null) { Add-Content -Path $sFullPath -Value "Maintainer $ScriptMaintainer"}
    ...
    Write-Debug "***************************************************************************************************"
    Write-Debug ""
    if ($ScriptName -ne $null) { Write-Debug "$ScriptName"}
    if ($ScriptMaintainer -ne $null) { Write-Debug "Maintainer $ScriptMaintainer"}
...
Function Log-Write{
  ...
  .PARAMETER Header
    Appends a *** line before and after the log entry
  ...
  .NOTES
    ...
    Version:        1.2
    Author:         Sten Lootens
    Creation Date:  28/11/16
    Purpose/Change: Added Header option
  .EXAMPLE
  ...
  Param ([Parameter(Mandatory=$true)][string]$LogPath, [Parameter(Mandatory=$true)][string]$LineValue, [Parameter(Mandatory=$false)][boolean]$Header)
